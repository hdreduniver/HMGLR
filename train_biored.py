...
from prepro import read_convert_biored
...
def train(args, model, train_features, dev_features):
    ...
    new_layer = ["extractor", "bilinear", "Linear", "gcn", "reason", "segmentation"]
...
def evaluate(args, model, features, tag="dev"):
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            ...
            preds.append(pred)
            golds.append(np.concatenate([np.array(label, np.int32) for label in batch[2]], axis=0))
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default="./biobert-base", type=str)
    ...
    parser.add_argument("--max_entity_number", default=50, type=int,
